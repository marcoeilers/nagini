...
class ContractTranslator(CommonTranslator):
    ...
    def translate_result(self, node: ast.Call, ctx: Context) -> StmtsAndExpr:
        ...
        if not ctx.actual_function.pure:
            ...
            # when checking transitivity, we substitute result with a local variable res
            if ctx.use_domain_func_eq and ctx.transitivity_result_var:
                return ([], ctx.transitivity_result_var)
    ...
    def translate_builtin_predicate(self, node: ast.Call, perm: Expr,
        ...
        if name == 'list_pred':
            ...
            if stmt:
                raise InvalidProgramException(node, 'purity.violated')
            perm_state_pred = self.viper.FullPerm(pos, self.no_info(ctx))
