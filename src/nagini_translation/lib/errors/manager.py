...
class ErrorManager:
    ...
    def _convert_error(
        ...
        if (error_item is not None and original_error.failureContexts().size() > 0 and
                original_error.failureContexts().head().counterExample().isDefined() and
                isinstance(error_item.py_node, PythonMethod)):
