...
def test_listpred(l: List[int]) -> int:
        """
static PyObject * test_listpred(PyObject *self, PyObject *args)
...
def test_listpred2(l: List[int]) -> int:
        """
static PyObject * test_listpred2(PyObject *self, PyObject *args)
requires PyExc(none, none) &*&
gil_lock(?gstate) &*&
hasRef(args, false) &*&
pyobj_hasval(args, PyTuple_v(cons(pair(?l__ptr, PyList_t(PyLong_t)), nil))) &*&
...
[1/3]list_forallpred(?l__content, pyobj_hasPyLongval, True, nil) &*&
(map(fst, l__content) == l__content__ptr) &*&
(some(map(snd, l__content)) == some(?l__content__val));
...
[1/3]list_forallpred(?NEW_l__content, pyobj_hasPyLongval, True, nil) &*&
(map(fst, NEW_l__content) == NEW_l__content__ptr) &*&
(some(map(snd, NEW_l__content)) == some(?NEW_l__content__val));
"""
...
def test_length(l: List[classA]) -> int:
        """
static PyObject * test_length(PyObject *self, PyObject *args)
requires PyExc(none, none) &*&
gil_lock(?gstate) &*&
hasRef(args, false) &*&
...
[1/3]pyobj_hascontent(l__ptr, List(?l__content__ptr)) &*&
[1/3]list_forallpred(?l__content, pyobj_hasPyClassInstanceval(PyClass_module_0classA()), True, nil) &*&
(map(fst, l__content) == l__content__ptr) &*&
(some(map(snd, l__content)) == some(?l__content__val)) &*&
...
pyobj_hasval(result, PyLong_v(?result__val)) &*&
[1/3]pyobj_hascontent(l__ptr, List(?NEW_l__content__ptr)) &*&
[1/3]list_forallpred(?NEW_l__content, pyobj_hasPyClassInstanceval(PyClass_module_0classA()), True, nil) &*&
(map(fst, NEW_l__content) == NEW_l__content__ptr) &*&
(some(map(snd, NEW_l__content)) == some(?NEW_l__content__val)) &*&
