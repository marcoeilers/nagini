...
def merge(r1: SR, r2: SR) -> SR:
    Requires(Acc(r1.runs) and Acc(r1.data) and list_pred(r1.runs) and list_pred(r1.data))
    Requires(Acc(r2.runs) and Acc(r2.data) and list_pred(r2.runs) and list_pred(r2.data))
    ...
    while ri1 < len(r1.runs) or ri2 < len(r2.runs):
        ...
        Invariant(Acc(r1.runs, 1 / 2) and Acc(list_pred(r1.runs), 1 / 2))
        Invariant(Acc(r2.runs, 1 / 2) and Acc(list_pred(r2.runs), 1 / 2))
        Invariant(Acc(r1.data, 1 / 2) and Acc(list_pred(r1.data), 1 / 2))
        Invariant(Acc(r2.data, 1 / 2) and Acc(list_pred(r2.data), 1 / 2))
...
def msort(a: List[int], l: int, h: int) -> SR:
    ...
    Assume(len(res.runs) > 0)
    Assume(len(res.data) == h - l)
    Assume(res.runs[-1] == len(res.data))
    Assume(Forall(int, lambda i: (Implies(i >= 0 and i < len(res.runs) - 1,
